...
　　夸张点说，Hive就是一个翻译工具，将SQL语句翻译成MapReduce程序去执行。

- Hive的系统架构

![Hive](/images/posts/Hadoop/Hive.png)

　　从此图中可以看出，Hadoop和mapreduce是hive架构的根基。Hive架构包括如下组件：CLI(Command line interface)、JDBC/ODBC、Thrift Server、WEB GUI、metastore和Driver(Complier、Optimizer和Executor)，这些组件可以分为两大类：服务端组件和客户端组件。
