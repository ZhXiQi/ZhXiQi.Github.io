...
>* Spout：描述了数据是如何从外部系统(或者组件内部直接产生)进入到Storm集群，并由该Spout所属的Topology来处理，通常是从一个数据源读取数据，也可以做一些简单的处理(为了不影响数据连续地、实时地、快速地进入到系统，通常不建议把复杂处理逻辑放在这里去做)。也就是说，在一个topology中，spout是获取源数据流的组件，通常情况下spout会从外部数据源中读取数据，然后转换为topology内部的源数据。

>* Bolt：这里是描述与业务相关的处理逻辑。bolt作为接收数据然后执行处理的组件，用户可以在其中执行自己想要的操作。
