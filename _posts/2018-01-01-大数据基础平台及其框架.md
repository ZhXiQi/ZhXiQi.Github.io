...
![DiffRDDOP](/images/posts/Hadoop/DiffRDDOP.png)

RDD的flatMap()和map()的区别：

![DiffMap](/images/posts/Hadoop/DiffMap.png)

因为转化操作返回的是RDD，所以不像普通的编程一样需要用字符串数组去接收切割的返回值

伪集合操作：以下四种集合操作都要求操作的RDD是相同数据类型的

![4RDD](/images/posts/Hadoop/4RDD.png)
...
![Cartesian](/images/posts/Hadoop/cartesian.png)

常见的RDD转化操作：

> 对一个数据为{1,2,3,3}的RDD进行基本的RDD转化操作

![tranRDD1](/images/posts/Hadoop/tranRDD1.png)

> 对数据分别为{1,2,3}和{3,4,5}的RDD进行针对两个RDD的转化操作

![tranRDD2](/images/posts/Hadoop/tranRDD2.png)

RDD行动操作：RDD的一些行动操作会以普通集合或者值的形式将RDD的部分或全部数据返回驱动器程序中。

> 对一个数据为{1,2,3,3}的RDD进行基本的RDD行动操作：

![actionRDD](/images/posts/Hadoop/actionRDD.png)
...
![JavaInterface](/images/posts/Hadoop/JavaInterface.png)
...
![RDDpersist](/images/posts/Hadoop/RDDpersist.png)
...
![Pair RDD](/images/posts/Hadoop/Pair RDD.png)

针对两个pair RDD的转化操作(rdd={(1,2),(3,4),(3,6)} other={(3,9)})

![Pair RDD2](/images/posts/Hadoop/Pair RDD2.png)

只访问pair RDD的值部分的话，Spark提供了mapValues(func)函数，功能类似与map{case(x,y):(x,func(y))}
```python
rdd.mapValues(lambda x:(x,1)).reduceByKey(lambda x,y: (x[0]+y[0],x[1]+y[1]))
```
![mapValues](/images/posts/Hadoop/mapValues.png)
